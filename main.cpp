...
template<typename fakeSocket> void closeConnection( fakeSocket* pConn )
{
        std::this_thread::sleep_for( std::chrono::milliseconds( 7 ) );
}

template<typename fakeSocket> bool recycleConnection( fakeSocket* pConn )
{
        std::this_thread::sleep_for( std::chrono::milliseconds( 7 ) );
...
int main( int argc, char **argv )
{
	...
	g_ConnPool.start_pool_managers();
